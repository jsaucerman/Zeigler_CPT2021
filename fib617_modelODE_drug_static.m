...
function dydt = fib617_modelODE_drug_static(t,y,params)
...
dydt = dydt.*KI';
%timp1ANDproMMP9 = AND(rpar(:,71),inhib(y(TIMP1),rpar(:,71)),act(y(proMMP9),rpar(:,71)));
%timp2ANDproMMP9 = AND(rpar(:,73),inhib(y(TIMP2),rpar(:,73)),act(y(proMMP9),rpar(:,73)));
% timp1Reaction = inhib(y(TIMP1),rpar(:,71));
% proMMP9Reaction1 = act(y(proMMP9),rpar(:,71));
% timp2Reaction = inhib(y(TIMP2),rpar(:,73));
% proMMP9Reaction2 = act(y(proMMP9),rpar(:,73));
% algvars = [timp1Reaction, proMMP9Reaction1, timp2Reaction, proMMP9Reaction2];

% activationByROS = act(y(ROS),rpar(:,38));
% inhibitionByPP1 = inhib(y(PP1),rpar(:,38));
% inhibitionByERK = inhib(y(ERK),rpar(:,107));
% activationByRAF = act(y(Raf),rpar(:,107));
% algvars = [activationByROS, inhibitionByPP1, activationByRAF, inhibitionByERK]; 
end
% utility functions 
